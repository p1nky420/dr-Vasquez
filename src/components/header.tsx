    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled || open ? "border-b border-white/10 bg-[#090806]/92 shadow-[0_18px_60px_rgba(0,0,0,.28)] backdrop-blur-xl" : "bg-gradient-to-b from-black/65 to-transparent"}`}>
      <div className="mx-auto flex h-20 max-w-[94rem] items-center justify-between px-5 md:h-24 md:px-8">
        <Link href="/" onClick={() => setOpen(false)} className="group flex items-center gap-4" aria-label="Fausto Vásquez Abogados, inicio">
          <span className="font-serif text-3xl font-semibold tracking-[-0.06em] text-[#c9ad78]">FV</span>
          <span className="h-7 w-px bg-[#c9ad78]/35" />
          <span>
            <strong className="block font-serif text-sm font-normal tracking-[0.04em] text-[#f3eee4] sm:text-base">Fausto Vásquez</strong>
            <small className="mt-1 block text-[0.58rem] uppercase tracking-[0.26em] text-[#b8b0a5]">Jurista penalista</small>
          </span>
        </Link>

        <nav className="hidden items-center gap-7 lg:flex" aria-label="Navegación principal">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="relative py-3 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-[#c2baae] transition-colors after:absolute after:inset-x-0 after:bottom-1 after:h-px after:origin-left after:scale-x-0 after:bg-[#c9ad78] after:transition-transform hover:text-[#f3eee4] hover:after:scale-x-100">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-3">
          <Link href="/#consulta" className="hidden border border-[#c9ad78]/55 px-5 py-3 text-[0.66rem] font-semibold uppercase tracking-[0.15em] text-[#f3eee4] transition-colors hover:bg-[#c9ad78] hover:text-[#15130f] md:inline-flex">
            Consulta privada
          </Link>
          <button type="button" onClick={() => setOpen((value) => !value)} aria-label={open ? "Cerrar menú" : "Abrir menú"} aria-expanded={open} aria-controls="mobile-navigation" className="grid size-12 place-items-center border border-white/15 text-[#c9ad78] lg:hidden">
            ...
            <nav id="mobile-navigation" className="grid" aria-label="Navegación móvil">
              {navItems.map((item, index) => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="flex min-h-16 items-center justify-between border-b border-white/10 font-serif text-xl text-[#f3eee4]">
                  {item.label}<span className="font-sans text-[0.65rem] text-[#c9ad78]">0{index + 1}</span>
                </Link>
              ))}
            </nav>
          </motion.div>
